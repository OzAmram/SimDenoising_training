...
def get_files(filetype):
    globstr = ""
    if filetype == 'Fuzz': globstr = '/storage/local/data1/gpuscratch/oamram/denoise/with_feats/ntup_photon_energy'+str(energy)+'*Prod*_n'+str(num_events)+'_part*.root'
    elif filetype == 'Sharp': globstr = '/storage/local/data1/gpuscratch/oamram/denoise/with_feats/ntup_photon_energy'+str(energy)+'*phi0.0_n'+str(num_events)+'_part*.root'
...
config.outf = 'out-train-may11-n20'
